...
main (int argc, char **argv)
{
  ...
  *p = '\0';
  ...
  if (opt.tui && nurls == 0)
    {
      TuiResult *res = tui_get_info();
      if (res && res->count > 0)
        {
          char **new_argv = xmalloc (sizeof(char *) * (argc + res->count + 1));
          memcpy (new_argv, argv, sizeof(char *) * argc);
          
          for (int i = 0; i < res->count; i++) {
              new_argv[argc + i] = res->urls[i];
          }
          new_argv[argc + res->count] = NULL;
  ...
  for (i = 0; i < nurls; i++, optind++)
    {
      ...
      if (!url_parsed)
      ...
      else
        {
          ...
          if (filename && expected_checksum) {
              FILE *fp = fopen(filename, "rb");
              if (fp) {
                  unsigned char digest[SHA256_DIGEST_SIZE];
                  if (sha256_stream(fp, digest) == 0) {
